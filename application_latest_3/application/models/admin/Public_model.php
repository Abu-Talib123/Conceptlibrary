...
class publics_model extends CI_Model
{
    ...
    // Category
    public function fetch_post_count() {
        $this->db->select("*");  
        $this->db->from("posts");
    ...
    public function fetch_posts($start_row, $limit) { 
        $qry = $this->db->query("SELECT * FROM posts WHERE is_deleted = 0 ORDER BY post_id ASC LIMIT $start_row, $limit");
        if ($qry->num_rows() > 0) {
    ...
    public function fetch_single_post($id) {  
        $upqry = 'SELECT * FROM posts WHERE post_id = \''.$id.'\' ';
    ...
    public function update_post_status($post_id, $is_deleted) {
        $this->db->where('post_id', $post_id);
        $this->db->update('posts', $is_deleted);
        return true;
    }

    public function post_title_exists($title) {
        $this->db->select('*'); 
        $this->db->from('posts');
