    ...
    <div class="container">
      <form role="form" name="frm_domain" id="frm_domain" action="#" method="post" enctype="multipart/form-data">
       ...
       <div class="row" style="margin-bottom:10px;">
        ...
        foreach($fetch_domaindata as $result)
          {
             
            $stu_id = $this->session->userdata('CL_STUDENT_ID');
            $exsqry =   $this->db->query('SELECT *  from payment  where student_id = \''.$stu_id.'\' AND material_id ='.$result['subcategory_id'].' AND (material_type = \''.$check_domain_type.'\' or  material_type = \''.$domain_type.'\') AND paymentstatus =\''.$paymentstatus.'\'');
             ...
             $status        = $this->mock_model->fetch_statusbysubcat( $result['subcategory_id']);
              ...
              <div class="col-lg-4">
                    <div class="course-1-item" style="max-height: 536px;">
                        ...
                        <?php
                          $today = date("Y-m-d");
                          //AND  substring(end_date,1,10) >= \''.$today.'\'
                          $query =   $this->db->query('SELECT *  from exam  where subcategory_id = \''.$result['subcategory_id'].'\'  AND is_active = 1  AND is_deleted =\'0\'');
                          $count = $query->num_rows();
                          if($count != 0){
                          ?>
                          <div class="productcount"> (<?=$count?>) Test Series</div>
                        <?php }?>
                        <?php if($result['status'] == 1){?>
                        <div class="price">&#8377;<?php if($result['m_offer_price'] > 0) { echo '<strike>'.$result['m_price'].'</strike><br/>'.'&#8377;'.$result['m_offer_price'];}else{ echo $result['m_price'];} ?></div>
                        <?php }else {?>
                          <div class="price">Free</div>
                       <?php }?>
                        <div class="category"><h2 ><?=$result['subcategory_name'];?></h2> 
                        ...
                        <div class="col-md-6">
                          <div class="course-1-content pb-4">
                              <p><a href="<?php echo base_url();?>mockpaper/mock_data/<?= $result['subcategory_id']; ?>" class="btn btn-primary rounded-0">View</a></p>
                          </div>
                        </div>
                        <div class="col-md-6">
                          <?php
                              ...
                              { 
                                $cartcontents = $this->Common_model->fetch_cart($this->session->userdata('CL_STUDENT_ID'),$result['subcategory_id'], $domain_type);
                                 if($cartcontents){
                                  ...
                                  { 
                                    if (($product['domain_type'] == $domain_type || $product['domain_type'] == 'all')  && $product['subcategory_id'] == $result['subcategory_id'] )
                                    { 
                                ?>
                                <div class="course-1-content pb-4">
